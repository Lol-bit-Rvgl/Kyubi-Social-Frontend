...
class WallScreen extends ConsumerStatefulWidget {
  const WallScreen({super.key, required this.ownerUsername});
...
class _WallScreenState extends ConsumerState<WallScreen> {
  ...
  int _page = 1;
  String? _error;
  bool _liking = false; // └🟢 protege contra doble clic en Like
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final page = await repo.getWall(widget.ownerUsername, page: 1);
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _loadMore() async {
    ...
    try {
      final repo = ref.read(wallRepositoryProvider);
      final page = await repo.getWall(widget.ownerUsername, page: _page + 1);
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _post() async {
    ...
    try {
      final repo = ref.read(wallRepositoryProvider);
      final entry = await repo.createEntry(widget.ownerUsername, text: text);
      if (!mounted) return;
      _controller.clear();
      setState(() {
  ...
  Future<void> _toggleLike(WallEntry entry) async {
    if (_liking) return; // └🟢 protege contra doble clic
    _liking = true;
    try {
      final repo = ref.read(wallRepositoryProvider);
      ...
      _liking = false; // └🟢 siempre liberar el flag
    }
  }
  ...
  Widget build(BuildContext context) {
    final isOwnWall =
        ref.read(authControllerProvider).user?.username == widget.ownerUsername;
    final title = isOwnWall ? 'Mi muro' : 'Muro de @${widget.ownerUsername}';

    return Scaffold(
      appBar: AppBar(title: Text(title)),
      body: Column(
        children: [
          if (isOwnWall) _buildComposer(),
          Expanded(
  ...
  Widget _buildContent() {
    ...
    if (_error != null) {
      return _InlineRetry(message: _error!, onRetry: _load);
    }
    if (_entries.isEmpty) {
      return _buildEmpty();
    }
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isOwnEntry =
            ref.read(authControllerProvider).user?.id == entry.authorId;
        return Column(
          children: [
            _WallTile(
              entry: entry,
              onLike: () => _toggleLike(entry),
              onDelete: isOwnEntry || _canDelete(entry)
                  ? () => _delete(entry)
                  : null,
            ),
            Divider(
  ...
  bool _canDelete(WallEntry entry) {
    ...
    if (entry.authorId == myId) return true;
    final isOwnWall =
        ref.read(authControllerProvider).user?.username == widget.ownerUsername;
    return isOwnWall;
  }
